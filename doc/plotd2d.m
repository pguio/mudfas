function [d2phi,expphi,err]=plotd2d(u,f,solver,argcaxis,mnp,func)
% function [d2phi,expphi,err]=plotd2d(u,f,solver,argcaxis,mnp,func)

%
% $Id: plotd2d.m,v 1.2 2011/03/26 12:56:38 patrick Exp $
%
...
d2phi=L(2:end-1,2:end-1);
d2phi=d2phi(2:end-1,2:end-1);

expphi=-nlF(u,Ne2d(solver,solver.ngrid),Te2d(solver,solver.ngrid));
expphi=expphi(2:end-1,2:end-1);

err=L(2:end-1,2:end-1)+ ...
	nlF(u,Ne2d(solver,solver.ngrid),Te2d(solver,solver.ngrid))-f;
err=abs(err(2:end-1,2:end-1));
...
imagesc(x,y,log10(errvis)'), axis xy 
colorbar('v');
if ~isempty(argcaxis),
...
subplot(m,n,p), imagesc(x,y,f), colorbar('h');
if mod(p-1,n)~=0, set(gca,'yticklabel',[]); end;
if mod(p-1,n)==0, ylabel(func); end;
if fix((p-1)/n)~=m-1, set(gca,'xticklabel',[]); end;
if fix((p-1)/n)==0, title('\rho'); end;

subplot(m,n,p+1), imagesc(x,y,d2phi), colorbar('h');
if mod(p,n)~=0, set(gca,'yticklabel',[]); end;
if fix(p/n)~=m-1, set(gca,'xticklabel',[]); end;
if fix(p/n)==0, title('\nabla^2\Phi'); end;

subplot(m,n,p+2), imagesc(x,y,-expphi), colorbar('h');
if mod(p+1,n)~=0, set(gca,'yticklabel',[]); end;
if fix((p+1)/n)~=m-1, set(gca,'xticklabel',[]); end;
if fix((p+1)/n)==0, title('-exp(\Phi/T)'); end;

subplot(m,n,p+3), imagesc(x,y,err), colorbar('h');
