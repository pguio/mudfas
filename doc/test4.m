...
%
% $Id: test4.m,v 1.2 2011/03/26 12:56:39 patrick Exp $
%
...
if dim==2,
	logfile='test4_2d.log';
	global nx ny
	global a kx ky X Y x y phit
elseif dim==3,
	logfile='test4_3d.log';
	global nx ny nz
...
if dim==2,
	fid=fopen('test4_2d.dat','w');
elseif dim==3,
	 fid=fopen('test4_3d.dat','w');
end
...
t=cputime;
if dim==2,
	[phi1,rho,solver]=mgfas2d('fun4');
	Te1=Te2d(solver,solver.ngrid);
elseif dim==3,
	[phi1,rho,solver]=mgfas3d('fun4');
...
if dim==2,
	if exist('exportfig'),
		exportfig(gcf,'test4_1_2d','format','eps');
	else
		print -deps test4_1_2d;
	end
elseif dim==3,
	if exist('exportfig'),
		exportfig(gcf,'test4_1_3d','format','eps');
	else
		print -deps test4_1_3d;
	end
end
